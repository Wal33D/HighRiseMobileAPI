...
class HighriseAPI {
  ...
  async getAccountInfo() {
    const requestBody = JSON.stringify({ "_type": "GetAccountInfoRequest" });
    return await this.getData(requestBody);
  }

  async getItems(page = 0, limit = 20, sort = 'date_descending', type = 'all', rarity = []) {
    const requestBody = JSON.stringify({ "_type": "GetNextjsItemsRequest", "page": page, "limit": limit, "sort": sort, "type": type, "rarity": rarity });
    return await this.getData(requestBody);
  }
  ...
  generateAndAppendHTML(items) {
    let html = '';
    for (var i = 0; i < items.length; i++) {
      html += `\n'${items[i].disp_name}' || ${items[i].rarity} || Market Price: ${items[i].acquisition_cost} ${items[i].acquisition_currency}`;
    }
    console.log(html)
    return items;
  }
